...
def train(experiment=EXPERIMENT):
    ...
    model = EmbeddingHead(embedding_dim=EMBEDDING_DIM).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)

    if config['type'] == 'augmentation_only':
        dataset = cache_data_loader.CachedPairDataset(augmentation_only=True, no_soft_positive=False)
    elif config['type'] == 'no_soft_positive':
        dataset = cache_data_loader.CachedPairDataset(augmentation_only=False, no_soft_positive=True)
    else:
        dataset = cache_data_loader.CachedPairDataset(augmentation_only=False, no_soft_positive=False)

    dataset.resample()
    loader = cache_data_loader.build_dataloader(dataset)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for feature1, feature2, weight in loader:
            feature1, feature2, weight = feature1.to(device), feature2.to(device), weight.to(device)
            emb1 = model(feature1)
            emb2 = model(feature2)
            ...
            epoch_loss += loss.item() * feature1.size(0)
            with torch.no_grad():
